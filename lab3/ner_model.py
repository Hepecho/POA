...
class BiLSTMCRF(nn.Module):
    ...
    def forward(self, sentences, lengths=None):
        ...
        """
        sentences = torch.as_tensor(sentences, dtype=torch.long)
        if not lengths:
    ...
    def __viterbi_decode(self, logits):
        ...
        viterbi = [torch.max(trellis[-1], -1)[1].to(device).tolist()]
        backpointers = backpointers.numpy()
        for bp in reversed(backpointers[1:]):
            viterbi.append(bp[viterbi[-1]])
        viterbi.reverse()

        viterbi_score = torch.max(trellis[-1], 0)[0].to(device).tolist()
    ...
    def __viterbi_decode_v1(self, logits):
        ...
        for index, logit in enumerate(logits):
            if index == 0:
                ...
                path.append(max_path.to(device).tolist())
                continue
            ...
            prev_score, max_path = torch.max(prev_prob, -1)
            path.append(max_path.to(device).tolist())
        return prev_score.to(device).tolist(), path
