...
def batch_process(batch):
    ...
    for i in range(len(text_list)):
        text_list_.append(
            torch.tensor([vocab[token] if token in list(vocab.keys()) else vocab[UNK] for token in text_list[i]]))
        aspect_list_.append(
            torch.tensor([vocab[token] if token in list(vocab.keys()) else vocab[UNK] for token in aspect_list[i]]))

    text_list_ = pad_sequence(text_list_, batch_first=True, padding_value=0)
    # padding数据 以最长为统一长度，并规定0维为batchsize, 0填充
